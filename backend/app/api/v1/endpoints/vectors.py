...
from app.db.session import get_db, get_vector_db
...
async def search_similar_conversations(
    ...
    try:
        ...
        # ベクトル検索実行
        search_service = create_vector_search_service(vector_db)
...
async def perform_clustering(
    ...
    """成功会話ベクトルのクラスタリング実行"""
    try:
        clustering_service = create_clustering_service(vector_db)
        ...
        background_tasks.add_task(
            ...
            vector_db, result['cluster_result_id']
        )
...
async def extract_cluster_representatives(
    ...
    """クラスタ代表例の抽出"""
    try:
        extraction_service = create_representative_extraction_service(vector_db)
...
async def get_clustering_results(
    ...
    try:
        ...
        results = vector_db.query(ClusterResult).order_by(
        ...
        return {
            ...
            "total": vector_db.query(ClusterResult).count()
        }
...
async def delete_clustering_result(
    ...
    try:
        ...
        # 関連データを削除
        vector_db.query(ClusterRepresentative).filter(
            ClusterRepresentative.cluster_result_id == cluster_result_id
        ).delete()
        
        vector_db.query(ClusterAssignment).filter(
            ClusterAssignment.cluster_result_id == cluster_result_id
        ).delete()
        
        result = vector_db.query(ClusterResult).filter(
...
# バックグラウンドタスク
async def extract_representatives_background(vector_db: Session, cluster_result_id: str):
    """代表例抽出をバックグラウンドで実行"""
    try:
        extraction_service = create_representative_extraction_service(vector_db)
